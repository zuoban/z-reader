...
export function CategoryManager({ onCategoryChange }: CategoryManagerProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent
        className="max-h-[85dvh] max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-[28px] border border-border/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(248,250,252,0.98))] p-0 shadow-2xl dark:bg-[linear-gradient(180deg,rgba(17,24,39,0.98),rgba(15,23,42,0.98))] sm:max-w-xl"
        closeButtonClassName="top-4 right-4 h-8 w-8 rounded-full border border-border/70 bg-background/90 shadow-sm backdrop-blur-sm hover:scale-105"
      >
        <div className="flex max-h-[85dvh] flex-col">
          <DialogHeader className="border-b border-border/60 bg-gradient-to-r from-muted/70 via-background to-muted/50 px-5 py-5 pr-14 sm:px-6">
                        ...
                        >
                          ...
                          <span
                            className={`pointer-events-none absolute inset-0 rounded-2xl ring-2 ring-offset-2 ring-offset-background transition-opacity ${
                              selected ? 'ring-foreground/70 opacity-100' : 'ring-transparent opacity-0'
                            }`}
                          />
                        </button>
                      );
