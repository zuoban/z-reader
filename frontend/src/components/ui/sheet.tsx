...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "paper-motion-veil fixed inset-0 z-50 bg-black/48 backdrop-blur-[2px] data-ending-style:opacity-0 data-starting-style:opacity-0 motion-reduce:duration-0",
        className
      )}
      {...props}
    />
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col gap-1.5 border-b border-border/45 bg-card/32 px-5 py-4 pr-14 shadow-[inset_0_1px_0_color-mix(in_srgb,var(--glass-specular)_34%,transparent)] ",
        className
      )}
      {...props}
    />
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "mt-auto flex flex-col gap-2 border-t border-border/45 bg-card/24 p-4 pb-[calc(1rem+env(safe-area-inset-bottom,0px))] shadow-[inset_0_1px_0_color-mix(in_srgb,var(--glass-specular)_28%,transparent)] ",
        className
      )}
      {...props}
    />
